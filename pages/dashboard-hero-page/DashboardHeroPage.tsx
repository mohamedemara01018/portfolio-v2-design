...
function DashboardHeroPage({ profileInfoData }: { profileInfoData: DashboardHeroPageProbs }) {
            ...
            <div className='mt-8 space-y-8'>
                <form onSubmit={(e) => {
                    ...
                    <div className='grid grid-cols-2 gap-8'>
                        <InputForm id={'email'} label='Email' placeholder="Your email" name='email' type='email' value={formData.email} handleChange={handleChange} />
                        ...
                        <InputForm id={'github'} label='Github' placeholder="Your github link" name='github' value={formData.github} handleChange={handleChange} />
                        <InputForm id={'linkedin'} label='LinkedIn' placeholder="Your linkedin link" name='linkedin' value={formData.linkedin} handleChange={handleChange} />
                        <InputForm id={'leetcode'} label='Leetcode' placeholder="Your leetcode link" name='leetcode' value={formData.leetcode} handleChange={handleChange} />
                        <InputForm id={'codeforces'} label='Codeforces' placeholder="Your codeforces link" name='codeforces' value={formData.codeforces} handleChange={handleChange} />
                    </div>
                    {/* image */}
                    <div className='flex flex-col items-start gap-1 w-full mt-4'>
                        <label htmlFor='avatar' >Avatar</label>
                        <input id={'avatar'} name='avatar' ref={file} onChange={handleFileChange} type="file" className='border border-border py-2 px-4 rounded-md bg-accent w-full focus:outline-(--portfolio-accent) ' />
                    </div>
                    <hr className='w-full h-0.5 bg-border' />
                    <button disabled={isSaving} className='py-2 px-4 rounded-md bg-(--portfolio-accent) hover:bg-(--portfolio-accent-hover) text-white w-fit disabled:opacity-50 transition-opacity'>
                        {isSaving ? 'Processing...' : (!isEmpty ? 'Save Change' : 'Create profile info')}
                    </button>
